...
Validation uses a local HTTPS npm registry with a private test CA and an HTTP CONNECT proxy. Actual Bun installs succeed through that proxy, and `NO_PROXY=localhost` bypasses it with a fresh package cache. See [Bun npmrc support](https://bun.sh/docs/pm/npmrc) and [Bun proxy configuration](https://github.com/oven-sh/bun/blob/main/docs/guides/http/proxy.mdx).
...
Tags are always resolved at the origin. Once a digest is known, Bunko tries the configured mirrors in order, then the origin. Digest-pinned roots can be fetched directly from a mirror. A missing object, exhausted connection retries, rate limiting or server error allows fallback; authentication/policy errors, corrupt content and interrupted bodies fail the operation. Digest and size verification remains mandatory when content is consumed.
