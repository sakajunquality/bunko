...
Cache tags use `bunko-cache-v1-<kind>-<full-key>` for app, deps, assets and runtime layers and, since v0.5.0, dependency closure plan indexes (`deps-plan`). They are written only with an explicit `--cache-repo`, `BUNKO_CACHE_REPO` or `--cache-to`. Plain publication does not create cache tags; legacy cache reads remain enabled. This default changed in 0.4.0. Cache artifacts publish after the image, in parallel under the same `--publish-concurrency` bound, each artifact placing its config and layer serially. Use `--cache-repo` or `BUNKO_CACHE_REPO` for a separate cache repository. Typed `--cache-to type=registry,repo=REPO` adds explicit cache exports; see [cache distribution](CACHE_RETENTION.md) for destination precedence and local backends. Unsupported custom OCI artifacts or denied cache writes warn by default; `--cache-export-error=fail` fails after recording outcomes without undoing successful image publication. Disable Registry caching with `--no-registry-cache`.

Blob placement uses HEAD, then an available same-Registry cross-repository mount, then upload. The blobs of one manifest are placed in parallel, at most `--publish-concurrency` at a time (`BUNKO_PUBLISH_CONCURRENCY`, 1-32, default 6). Docker Hub meters requests per account and answers a burst with 429/503, so it defaults to 3. A 429/503 is retried after the registry's `Retry-After` on reads and on upload-session creation, chunk recovery and finalization, and that pause is shared per registry: a parallel worker that meets the limit while a pause is running waits for that pause instead of starting another one. Lower the value further on an account that still sees 429/503s. Parallel placement against cloud registries is verified against the mock endpoint only; the live matrix above was recorded before this change. Uploads normally use 8 MiB chunks and offset reconciliation. GHCR and Artifact Registry use a streamed, full-file PUT; a streamed full-file PUT holds one slot for the whole transfer, so a large image on a narrow uplink may want a lower bound. Live tests observed GHCR rejecting the ranged PATCH and Artifact Registry rejecting a second chunk. Interrupted full-file PUTs are checked by digest before retrying in a fresh session; permanent permission failures are not retried. [Artifact Registry API support](https://docs.cloud.google.com/artifact-registry/docs/reference/docker-api) Publish platform manifests and the index by digest before updating tags. Multiple tags are not transactional: `--report` records published digests/tags and pendingTags on failure, with exit 1 and empty stdout. Existing tags are not rolled back.
...
The workflow uses the official [Docker login action](https://github.com/docker/login-action), [Google authentication action](https://github.com/google-github-actions/auth), [AWS credentials action](https://github.com/aws-actions/configure-aws-credentials), and [ECR login action](https://github.com/aws-actions/amazon-ecr-login), pinned to commits. Create repositories and configure permissions beforehand. Supply a separate cache repository on the same host to exercise cross-repository cache reuse. When cache verification is required and no cache repository is configured, the conformance harness explicitly passes the image repository as its cache write destination in both source and CLI runs. Ordinary builds still need an explicit `--cache-repo`, `BUNKO_CACHE_REPO` or registry `--cache-to` to write cache tags; an omitted destination only permits legacy cache reads. ECR repositories must allow the distinct cache/image tags created by repeated runs.
...
Cache export error policy is separate from image publication tag policy: `--cache-export-error` defaults to `warn`, while `--tag-conflict` defaults to `fail` for image tags. Cache exports reconcile verified identical per-key records regardless of the image tag policy.
...
Registry requests bound response-header waits for reads and writes. Upload streams reset an idle timer as the HTTP transport consumes chunks, then use the header deadline after EOF; there is no total upload-duration limit. Transport consumption is backpressure evidence, not proof the remote registry has committed bytes. A failed write is never blindly replayed by the registry client; the publisher's upload-state/digest reconciliation decides whether resuming is safe.
