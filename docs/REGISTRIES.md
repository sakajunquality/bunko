...
Cache tags use `bunko-cache-v1-<kind>-<full-key>` for app, deps, assets and runtime layers, and are written only with an explicit `--cache-repo`, `BUNKO_CACHE_REPO` or `--cache-to`. Plain publication does not create cache tags; legacy cache reads remain enabled. This default changed in 0.4.0. Cache artifacts publish after the image, in parallel under the same `--publish-concurrency` bound, each artifact placing its config and layer serially. Use `--cache-repo` or `BUNKO_CACHE_REPO` for a separate cache repository. Typed `--cache-to type=registry,repo=REPO` adds explicit cache exports; see [cache distribution](CACHE_RETENTION.md) for destination precedence and local backends. Unsupported custom OCI artifacts or denied cache writes warn by default; `--cache-export-error=fail` fails after recording outcomes without undoing successful image publication. Disable Registry caching with `--no-registry-cache`.
