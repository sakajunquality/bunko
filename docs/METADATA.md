...
Supply-chain attestations are opt-in. `--sbom --provenance` publishes OCI subject artifacts alongside the runnable image and includes them in an OCI layout. They do not change the runnable image digest. The host Bun compressor version and the builder fingerprint do participate in image identity and application cache keys.
...
Artifact publication uses a verified manifest PUT's `OCI-Subject` acknowledgement to select native referrers discovery. Without that acknowledgement, Bunko updates the standard `sha256-<subject>` fallback index, even if a referrers GET endpoint would return an empty success response. Wrong subject acknowledgements fail. An acknowledged API must retain the artifact; an empty response does not silently count as success.

Fallback index read/merge/write operations are serialized per registry repository and subject within a Bunko process, including separate library publishers. Independent processes must coordinate publication externally: OCI tags do not provide a universal atomic compare-and-swap. Readback detects observed lost updates, but cannot prevent a later external writer from replacing a tag.
...
New builds record a versioned JSON capsule in the reserved `org.bunko.rebase.metadata` image-config label. It identifies the selected base manifest and config, the optional base index, the base layer boundary, generated layer roles, platform, build toolchain, runtime origin, and configuration ownership policies. The config digest binds this capsule to the image. It changes image identity but does not add or alter filesystem layers.

Ownership distinguishes inherited environment variables and labels from explicitly supplied keys, including overrides whose values equal the base. It also records the nonroot user fallback, explicit versus inherited ports, command/workdir replacement, environment defaults, and base-label filtering policies. Runtime certificate environment keys generated by Bunko count as owned overrides. Values remain in the ordinary image config; the capsule does not duplicate them or record host source paths. The canonical UTF-8 JSON is limited to 64 KiB; exceeding that limit fails the build.

The [rebase command](REBASE.md) validates this capsule against the original base and reconstructed configuration before replacing layers. The capsule itself does not certify ABI compatibility or rebase eligibility. The recorded Bun version and revision identify the build toolchain; ordinary builds do not independently verify custom base runtimes, so rebase checks the executable before accepting it. Older images without this capsule require rebuilding first.

Keyless mode requires stable cosign 3.x and explicit certificate identity/issuer constraints for verification. [Keyless signing](KEYLESS_SIGNING.md) describes trust configuration, disclosure, Action inputs and bounded token handling.
