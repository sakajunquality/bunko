...
This is preparatory metadata for [safe rebasing](https://github.com/sakajunquality/bunko/issues/54). There is no rebase command yet, and the capsule does not certify ABI compatibility or rebase eligibility. The recorded Bun version and revision identify the build toolchain; a runtime supplied by a custom base remains unverified. A future consumer must verify the original base and layer boundary, reconstruct configuration ownership, enforce compatibility and path-safety gates, and regenerate affected attestations before publishing a rebased image. Older images without this capsule cannot supply these ownership distinctions reliably.
