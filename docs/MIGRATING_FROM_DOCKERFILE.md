...
| `USER bun` | `bunko.user`, `--image-user` | Precedence: explicit setting, nonempty base User, then `65532:65532`. `oven/bun:*-distroless` declares `User` `0`, and an explicit base root user is preserved, so set `"user": "65532:65532"` explicitly when using the default base. |
...
| `COPY --from=... /etc/ssl/certs/ca-certificates.crt` | usually none | `oven/bun:*-distroless` ships `/etc/ssl/certs/ca-certificates.crt` and sets `SSL_CERT_FILE` to it, which Bun and Go binaries honor. Private CAs for the application go in `bunko.runtime.caCertificates` (`NODE_EXTRA_CA_CERTS`, Bun/Node trust only). |
