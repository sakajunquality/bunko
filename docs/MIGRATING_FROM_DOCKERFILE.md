...
| `USER bun` | `bunko.user`, `--image-user` | Use numeric `uid[:gid]`. Explicit settings take precedence; otherwise only canonical nonzero numeric base UIDs are inherited, falling back to `65532:65532`. Named base users are not trusted. Set `"user": "0:0"` explicitly only when root is required. |
