...
The follow-up confirmed the implementation fixes. Its test-path finding was corrected by giving the offline build an explicit layout output; the complete local persistence path now passes. A present registry manifest with a missing config/blob is classified as invalid/unavailable, with a regression test.
