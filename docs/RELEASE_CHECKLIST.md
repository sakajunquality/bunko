...
- [ ] Use the release workflow's preparation Bun version (currently 1.3.13). Put that verified executable directory first on PATH and check `bun --version`; invoking an absolute Bun path alone does not ensure nested `bun run` scripts use the same version. Run `bun install --frozen-lockfile --ignore-scripts`, `bun run check`, and `bun run release:prepare /tmp/bunko-<version>-candidate` with a new destination outside the working tree. Record the CLI size, SHA256 and Bun version. Run the applicable application/runtime fixtures on both supported Linux architectures with that exact CLI.
...
When the `version` input is omitted, the existing `sakajunquality/bunko@...` setup entry point derives its CLI version from its own tag ref or checked-out `package.json`, so the new tag installs the new release as soon as it is published. An explicit `version` input takes precedence and must be updated separately; this existing entry point needs no CLI-default bump. The dedicated `sakajunquality/setup-bunko` Marketplace Action instead has an explicit CLI default and an independent Action release number; its update is a separate promotion step after CLI acceptance. `bun-version` and container digests remain literals and stay on the previously verified values until the new artifacts pass acceptance.
...
- [ ] Open a follow-up PR updating workflow version inputs, installation examples and digest-pinned container recipes. The existing root Action's CLI default follows its own ref, so this repository's `action.yml` and `scripts/setup.ts` literal need no CLI-default bump. Keep older version evidence intact; document that Action tags predating ref-derived defaults keep their original defaults and need explicit inputs.
- [ ] In `sakajunquality/setup-bunko`, open a separate reviewed PR updating the pinned installer commit, explicit CLI default, README and consumer expectations to the accepted CLI release. Verify input/output parity and Linux/macOS installation, version overrides, checksum/provenance rejection and local distributions. Publish a new independent Action version after CI passes, verify its Marketplace listing and published-tag consumer, then update bunko documentation pins. Never move an existing Action tag or republish CLI/npm/container artifacts for an Action-only release. If deferring this promotion, record the wrapper's still-current default explicitly.
