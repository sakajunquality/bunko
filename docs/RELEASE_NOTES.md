...
CLI cancellation aborts network work, stops queued work, signals owned subprocess groups and drains them before deleting registered temporary directories. SIGINT returns 130 and SIGTERM returns 143. A hard cancellation deadline retains scratch when work has not drained. Permission-denied process-group probes remain pending until exit can be confirmed, including the macOS group-exit race. Library calls do not install process signal handlers.
