...
Use a digest-pinned base for reproducible builds. An explicit base (configuration, CLI, BUNKO_DEFAULT_BASE, or local OCI layout) is required. The initial implementation supports bundle mode, Linux amd64/arm64, and official Bun 1.3.11, 1.3.12 and 1.3.13 toolchains. It uses x64-baseline for amd64 and aarch64 for arm64. Compile mode, musl, unsupported versions and custom revisions that cannot be matched to the release are rejected. Injection does not change the default base, install native-addon libraries, add Node.js/shell tools, or run package hooks.
...
Verified downloads are cached under `$XDG_CACHE_HOME/bunko/runtime/v1`, or `~/.cache/bunko/runtime/v1`; override with `--runtime-cache DIR`. This is separate from `--install-cache` and the layer cache. `--no-local-cache` also disables the build's persistent runtime download cache. Cache hits reverify signatures and archive bytes. Atomic writes and a per-release/architecture lock prevent cooperating writers from publishing partial entries. A contending download can wait up to 35 minutes; crashed locks are not removed automatically. The normal cache-info/prune commands manage layer caches, not this separate download cache.
