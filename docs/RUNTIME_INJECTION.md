...
Use a digest-pinned base for reproducible builds. An explicit base (configuration, CLI, BUNKO_DEFAULT_BASE, or local OCI layout) is required. Bunko v0.12.5 supports bundle/source modes, Linux amd64/arm64, and official Bun 1.3.13 and 1.4.2 toolchains. The immutable rc.3 release predates Bun 1.4 support. It uses x64-baseline for amd64 and aarch64 for arm64. Compile mode, unsupported versions and custom revisions that cannot be matched to the release are rejected. Injection does not change the default base, install native-addon libraries, add Node.js/shell tools, or run package hooks.
