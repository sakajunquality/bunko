...
On main after promotion, setup defaults to the verified rc.5 release and Bun 1.4.2. Older Action commits, including the immutable rc.5 tag, retain their original defaults; keep both version inputs explicit when pinning them. Attestation verification is opt-in and is available for rc.4 and later; see [release provenance](RELEASE_PROVENANCE.md).
