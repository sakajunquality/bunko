...
steps:
  ...
  - uses: sakajunquality/setup-bunko@2f4d5712dcafc7a1dd6488769b1e2806de33b516 # v0.2.0
    with:
      version: v0.12.4
      bun-version: 1.4.2
  - uses: sakajunquality/bunko/build@f3b5b66758b725b1eb793237f5fa3f75ebc01a41 # v0.12.4
    env:
...
Report strings are application-controlled, so the Action bounds and rewrites them before rendering. Build and command failure reports also redact credential-shaped text and random Bunko scratch roots before they are written. Free-form values (target names, phase names, cache reasons, error text) are cut to 200 characters (a token extending beyond the redaction window is omitted entirely), Markdown structure characters are escaped so a report cannot inject a link, image or heading, and row counts are bounded (40 phase rows per target, 20 cache and layer kinds per line, 50 targets); the rendered section is then held below GitHub's 1 MiB step summary limit and says so when it was cut short.
