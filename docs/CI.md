...
steps:
  ...
  - uses: sakajunquality/bunko@45f2daab119ed5d378f315c75afb73b322014086
    with:
      bun-version: 1.4.2
  ...
  - uses: sakajunquality/bunko/build@45f2daab119ed5d378f315c75afb73b322014086
    id: image
    with:
      path: .
...
The build Action appends a `### bunko build` section to `$GITHUB_STEP_SUMMARY`: the CLI version and the built target(s), a per-phase table of completed timings (one row per phase, summed across runs, slowest first, with a platform column when the build covers more than one platform), then one line each for cache status per kind, generated layer sizes with the stored total, and publication counts with uploaded bytes and elapsed time, followed by the published immutable reference when the build published one. Multi-target builds get one block per target. This replaces the per-workflow `jq` steps that read the same numbers out of the report.

The `## Bunko build` table of targets, digests and published references that the Action already wrote is retained above the new section on successful builds.

The section is rendered from the build report, so it is also written when the build fails and a report exists; a failed report adds the error and the targets that were not built. A missing, oversized or unparsable report degrades to a single note. Summary rendering never changes the result of the job: a step summary that cannot be written is reported on stderr and the build result stands.

Report strings are application-controlled, so the Action bounds and rewrites them before rendering. Free-form values (target names, phase names, cache reasons, error text) are cut to 200 characters (a token extending beyond the redaction window is omitted entirely), Markdown structure characters are escaped so a report cannot inject a link, image or heading, and row counts are bounded (40 phase rows per target, 20 cache and layer kinds per line, 50 targets); the rendered section is then held below GitHub's 1 MiB step summary limit and says so when it was cut short.

Free-form values also pass through the same credential scrubbers the CLI uses for installer output: URL userinfo (`scheme://user:password@host`) and URL query strings are replaced, `Authorization`, `Bearer` and npmrc credential values are masked, and known npm and GitHub token shapes are replaced, plus credential-bearing query keys (`token`, `access_token`, `sig`, `signature`, `key`, `password`, `secret`, `authorization`) in URLs written without a scheme. Registry hosts, repositories, digests and references are kept as-is. This is a scrub of known credential shapes in report content, not a guarantee about arbitrary text: no environment variable or Action input is ever read into the summary, and the report file itself remains unredacted for the artifact upload.

Set `summary: 'false'` to write nothing to `$GITHUB_STEP_SUMMARY`. The default is `'true'`, and nothing is written outside GitHub Actions, where `GITHUB_STEP_SUMMARY` is unset. The input requires an Action commit that includes it.
...
On GitHub Actions, set `cache: github` and the build Action persists the managed cache itself; see [the GitHub Actions cache](#the-github-actions-cache) below. On other providers, persist a local `cache-dir` with your CI cache service, or use explicit registry cache repositories. A registry cache carries the closure plan index as well as the layers, so a fresh runner reaches the warm dependency-closure path from `cache-repo` alone and restoring `cache-dir` is an optimization rather than a requirement; the CI cache service is still worth using for `install-cache`, Bun's package download cache. Persist `install-cache` separately for package downloads; in CLI 0.1.1 its default is `${XDG_CACHE_HOME:-~/.cache}/bunko/install/v1`. Inputs are not shell-expanded, so write cache paths without `~`, for example `${{ runner.temp }}/bunko/cache`. Cache keys should separate operating systems and Bun versions; Bunko validates its own content keys before reuse. Do not expose write credentials to untrusted pull requests. Use provider OIDC login steps for Artifact Registry or ECR, or a Docker Hub access token via `docker login`; see [registry authentication](REGISTRIES.md). Provider helper binaries are the workflow's responsibility.
...
The default key is `bunko-<runner.os>-<runner.arch>-<installed bunko version>-<targets>-<hash>`, where the hash covers `<path>/bun.lock`, `<path>/bun.lockb` and every `<path>/**/package.json` outside `node_modules`, and `<targets>` identifies the selected targets: the literal `all` when `targets` is empty, otherwise the first 12 hex characters of a SHA-256 digest of the selection, sorted and de-duplicated, so the order and any repetition in the input do not change the key. The restore keys fall back to the same prefix without the hash, so a manifest change still starts from the previous run's cache, and the targets segment sits inside that prefix so a prefix match restores only an entry written under the same segment. `cache-key` sets an exact key instead, and `cache-restore-keys` (one prefix per line) replaces the default fallback. Cache keys are limited to 512 characters and may not contain commas or whitespace. The Bun runtime version is deliberately not part of the key: Bunko validates its own content keys before reuse, so entries a Bun upgrade invalidated miss instead of being reused, and the package download cache stays shared. Put it in `cache-key` when you want separate entries per Bun version.

The targets are in the key because the hash alone cannot separate two apps of one workspace. A monorepo with `@ezo-mobile/backend` and `@ezo-mobile/admin` gives each app its own deploy workflow, and both hash the same lockfile and the same set of `package.json` files. Run from one commit, they computed one identical key, raced to reserve it, and the second job to finish failed its save with `Unable to reserve cache with key …, another job may be creating this cache`: the loser persisted nothing and started its next run cold. The two caches also hold genuinely different content — one target's dependency closure was 206 packages, the other's empty — so a single key could never have represented both. With the segment, the two jobs write `bunko-Linux-X64-0.5.0-ecc9c1b12c83-<hash>` and `bunko-Linux-X64-0.5.0-50c3d96be4b1-<hash>`, and neither one's restore prefix reaches the other's entry. The segment is a 12-character truncation of a SHA-256 digest, so two different selections sharing one segment is possible rather than impossible; at 48 bits it is around 2e-9 for a thousand distinct selections in one repository, and `cache-key` settles it outright if that matters to you.

Adopting the segment costs every workflow one cold cache, not only the ones that select targets: an entry saved under the older `bunko-Linux-X64-0.5.0-<hash>` shape matches neither the new `bunko-Linux-X64-0.5.0-all-<hash>` key nor its `bunko-Linux-X64-0.5.0-all-` restore prefix, so the first run after the upgrade restores nothing and saves a fresh entry. Changing which targets a workflow selects costs one cold run the same way. The segment also multiplies entries, which is the honest price of not colliding: a workspace whose apps each have their own workflow now stores one cache per selection instead of one for the repository, using more of the repository's 10 GB cache allowance and evicting least-recently-used entries sooner. Point several workflows at one `cache-key` if you would rather have them share a single entry, and accept that only one of them will win the race to save it.
...
Registry closure plan transport requires CLI v0.5.0 or later; published v0.4.0 carries plans only in the local cache. `cache: github` and the registry cache compose. A registry cache (`cache-repo`, `cache-from`, `cache-to`) carries built layers and closure plan records between runners, repositories and developer machines, and survives GitHub's cache eviction; `cache: github` carries the parts that never reach a registry — base inspection records, the Bun package download cache and the image/URL asset caches — but only for the same repository and only until GitHub evicts the entry. With both configured, a fresh runner restores the local caches from GitHub, then pulls any layer it still misses from the registry cache, and reuses matching inputs while rebuilding changed application content:
