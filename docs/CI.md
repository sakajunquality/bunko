...
Report strings are application-controlled, so the Action bounds and rewrites them before rendering. Free-form values (target names, phase names, cache reasons, error text) are cut to 200 characters, Markdown structure characters are escaped so a report cannot inject a link, image or heading, and row counts are bounded (40 phase rows per target, 20 cache and layer kinds per line, 50 targets); the rendered section is then held below GitHub's 1 MiB step summary limit and says so when it was cut short.
