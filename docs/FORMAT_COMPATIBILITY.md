...
Run `bun scripts/validation/cache-upgrade.ts 0.10.0` and `bun scripts/validation/cache-upgrade.ts 0.11.0` with the pinned commits available in local Git history. The script extracts immutable released source and its lockfile, installs the exact historical dependency graph in that isolated directory with lifecycle scripts disabled, and executes the historical source CLI. This is not an npm installation smoke test. Installation may download public dependencies. It never links the current checkout's dependency tree into the historical CLI.
