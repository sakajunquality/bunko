...
A bounded Claude follow-up found no defects in float-tag handling, alias/merge resolution or document boundaries. Its one additional finding was fixed: labels must be an actual mapping (including alias resolution), so scalar wrapper objects and YAML sets cannot masquerade as label maps. Null labels remain equivalent to absent labels.
