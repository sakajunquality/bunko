...
Changes after rc.4 derive each [SPDX document namespace](https://spdx.github.io/spdx-spec/v2.3/document-creation-information/#65-spdx-document-namespace-field) from the canonical document content with the namespace field omitted. Different inventories for the same image therefore have distinct namespaces, while identical documents retain identical identities. The creation timestamp continues to use the normalized build epoch (zero unless configured), preserving deterministic metadata.

Provenance subjects use the effective destination repository, or `bunko.local/<name>` for a local image. External parameters include sorted define keys, a count and SHA-256 hash of runtime argv, runtime path/injection/certificate count, and asset exclusion/mode policy. Runtime argument and define values remain omitted; hashes do not make low-entropy values secret.

Asset mappings appear as external parameters and as resolved dependencies with their frozen content digest, annotated with the target platforms each material was resolved for. External sources add the identity of what was fetched: an `image` mapping records its reference and the resolved platform manifest digest, and a `url` mapping records the URL and the declared, verified `sha256`. A mapping that resolves differently per platform appears once per distinct result, so the digests are never ambiguous. Local context mappings continue to record logical names and relative paths only, never host directories. These values are public metadata when provenance is published.

Automatic Git labels include the revision, a dirty flag only when status succeeds, and a source URL derived from the origin remote. Common HTTP(S), SSH and Git remotes are normalized without userinfo, query parameters, or fragments; SSH/Git transports use a best-effort HTTPS source link without their transport port. Unsupported/local remote formats are omitted. Repository identifiers remain visible when Git metadata is enabled. `--git-metadata=false` disables these labels. If a checkout is detected but Git metadata cannot be read, a generic warning points to ownership and [Git safe.directory](https://git-scm.com/docs/git-config#Documentation/git-config.txt-safedirectory) configuration; Bunko neither prints Git stderr nor changes the trust policy. Trust only the specific checkout you control.
