...
| `inactiveNativeVariants` | Opposite-libc addon variants with a matching alternative; includes the addon path, libc, inferred base libc and alternative path. Files remain packaged. Conventional labels such as `linux-arm64`/`linuxmusl-arm64` and `linux-x64-gnu`/`linux-x64-musl` are recognized when their ELF dependencies agree. |
