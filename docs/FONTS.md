...
`check-config` inspects selected names, types, modes and sizes without reading font contents; 0.4.0 adds `--deep` to validate local font bytes too. Builds validate the staged bytes before hashing or packing, including on asset-cache hits.
