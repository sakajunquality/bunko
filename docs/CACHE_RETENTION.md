...
Every read cache and its writers must be trusted as much as the build inputs. A closure plan record is part of that boundary: a writer who controls the local cache directory can point a plan at a different closure layer, which is the same capability as supplying that layer's contents directly. A base inspection record is part of it too: it describes what an immutable base digest contains, so a writer who controls the directory can make workdir, data-destination and runtime-placement checks reason about a base other than the one that is pinned. Plan and base inspection records are read only from the local cache, never from a registry. A digest and DiffID prove content integrity and internal consistency; they do not prove that a producer built the layer from the claimed cache inputs. A malicious cache writer can supply application/dependency contents and inventory, and promotion preserves that content in the write destination. Producer signature verification for prepared dependencies does not authenticate application caches. Use isolated trusted cache repositories, or disable caches when that trust cannot be established. The CI policy does not elevate cache trust.
