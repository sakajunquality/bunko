...
New locks expose owner diagnostics and cap filesystem acquisition waits at five minutes. Local inspection reports unreferenced and temporary blob bytes separately; these are not currently reclaimed automatically. A foreign packing fingerprint alone no longer makes a closure plan immediately disposable. See [cache evolution](CACHE_EVOLUTION.md) for safe manual recovery, proposed lease/layout changes and the remaining cross-version retention work.

### Crash residue and staged writes

Layer writes copy and authenticate their blob in a private `.bunko-stage-<uuid>` directory before taking the shared metadata lock. Only final blob placement and record publication hold that lock. Each stage has a separate OS-backed lease; a slow writer therefore does not block ordinary cache metadata operations.

Local prune previews owned unreferenced CAS blobs and UUID-named temporary blob files older than both `--older-than` and one hour. The minimum hour also applies with `--older-than 0` or a byte budget. Recent files and unrecognized names are retained. Staging directories additionally require a confirmed dead local owner and a matching exclusive lease; live, foreign, incomplete and uncertain stages are retained. `--execute` performs the previewed reclamation. There is no automatic build-time GC.

`unreferencedBytes` and `temporaryBytes` count loose CAS residue. `residue` and `residueBytes` list reclaimable loose files and abandoned stages separately from `bytes` / `remainingBytes`, which continue to describe the managed retention budget. `cache-info` exposes the loose-file counts and `reclaimableResidueBytes`. These are not total filesystem usage: unknown files, uncertain stages and filesystem overhead remain outside accounting.

### Cache identities across CLI versions

Packing uses a hand-versioned `tar-gzip-v4` identity. A bunko patch release or the host Bun patch no longer invalidates assets and injected runtime layers merely by changing the producer version. Explicit asset/runtime/dependency policy revisions cover semantic transformations. Selected asset entries retain content, mode and destination identity; runtime entries retain authenticated runtime metadata and target platform. Dependency keys retain their installer/lock/platform inputs or projected content, and application keys retain the exact builder and toolchain identities. This does not promise cross-version reuse for compiler-dependent output.

New build records include diagnostic `writer` metadata (`bunko`, `bun`, `revision`). A writer-only difference does not make otherwise identical registry publication conflict. Payload verification and every other record field still participate in idempotency checks. Older records without writer metadata remain readable when their schema, packing identity and key match. The transition from the old version-qualified packing identity causes one cold build; old entries are retained for rollback and ordinary retention, not rewritten or promoted under unverified new keys.

### Mixed-version retention

New managed layer-cache writes add `bunko-cache.json` with `schemaVersion: 1`, `layoutVersion: 1`, and `minReader: 1`. `minReader` names the cache-layout reader protocol, not a CLI release. An absent envelope remains the legacy v1 layout. An unsupported envelope is a read miss and prevents record writes or deletion; prune reports it under `unmanaged`. Writes replace the small envelope atomically.

Local prune and cache-info list unfamiliar namespaces, filenames and future record revisions under `unmanaged`, without traversing unknown subtrees. Malformed records in a known schema still fail validation. Whenever unmanaged data could hide references, every CAS blob and crash stage is retained (`orphanReclamationSkipped: true`); known metadata may still be selected by age/budget. Its retained blobs remain in `remainingBytes`. Future base-inspection namespaces are retained rather than treated as obsolete. Older supported records are ordinary retention candidates, regardless of writer version.

Use `bunko prune --cache-repo ghcr.io/example/cache --keep-current` to preview deletion of validated older packing-format tags while retaining the current format and supported plan layout. Obsolete closure-plan-v1 tags remain candidates even if their packing string matches. Add `--execute` to delete only selected tags. Future cache record revisions are listed under `unmanaged`; unrelated content under a cache-shaped tag still fails validation. Registry creation timestamps and reclaimed bytes are not portable OCI metadata, so age-based remote retention remains a registry-provider policy.
