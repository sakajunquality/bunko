...
Repository release immutability was enabled on 2026-09-14. It applies to newly published releases; existing published releases are not retroactively locked. The active `Protect release tags` ruleset independently prohibits updates and deletion of `v*` tags without bypass actors. Upload and verify every asset before publishing a release; immutable release assets cannot be repaired in place. Publish a new version for corrections.
