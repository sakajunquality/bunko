...
`bunko.build.moduleLocations: "error"` or `--module-locations=error` fails the build after listing the warnings and the hint. The default remains `"warn"`. Independently of that setting, bunko rejects emitted JavaScript containing the temporary build snapshot path. Such inlined `__dirname` / `__filename` values are nondeterministic and point outside the image. Externalize the affected dependency, use source mode, or replace the reference with an explicit runtime asset root. This check also runs before compile mode turns the bundle into an executable.
...
networkConcurrency = 8 # Optional: reduce bursts against a private registry or proxy.

[test]
# Test settings are ignored by image builds. No test preload is executed.
```

Only the listed install settings are accepted. Age-gate settings are forwarded in the controlled installer TOML and included in dependency cache identity; `networkConcurrency` is passed only as a CLI flag and is excluded from dependency and closure-plan cache identity. Workspace install settings belong at the root. Bun's age gate affects new resolution; Bunko does not re-audit the publication age of already locked versions. Other install settings, registry/authentication entries, runtime preloads, and unknown top-level settings are rejected. Continue using the supported root `.npmrc` credential mechanism. Diagnostics identify unsupported keys without printing their values.

Both the host build install and Linux production install use `install.networkConcurrency` (an integer from 1 to 65535). If it is absent, bunko accepts `BUN_CONFIG_NETWORK_CONCURRENCY` with the same range; if neither is set, Bun keeps its default. The project setting takes precedence. This limit is per installer process, so parallel targets and CI jobs can still exceed a shared proxy's rate limit. Both forms are transport-only and are not dependency cache inputs. An empty environment variable is treated as unset. Editing the source `bunfig.toml` may still invalidate an application snapshot cache; it does not invalidate a matching dependency closure plan.

Recognized transient download errors (connection refusal/reset/timeout, HTTP 429 and 5xx) receive at most two retries, after one and two seconds, with the same download cache and frozen inputs. Cancellation interrupts the wait. Authentication, missing packages, integrity, lockfile, certificate and unrecognized errors are not retried; an unrelated error alongside a transport error also prevents retry. Exhausted retries include a rate-limit/proxy hint and the existing credential-redacted installer output. No retry can repair a persistent registry failure; reduce concurrency for that registry or adjust the proxy limit. Bun may also perform its own network retries within each attempt. Some Bun download failures expose only `HTTP 4xx`, hiding the actual status; bunko does not retry that ambiguous diagnostic because it could represent an authentication or missing-package error. Use the concurrency setting proactively for these registries.
...
The finding is then neither logged nor counted; the closure reports `Acknowledged 1 undeclared import(s): @babel/core@7.27.7 -> @babel/preset-typescript` instead. An entry that matches no finding — because the package was fixed or the version moved past a pinned entry — logs `BUNKO_UNUSED_ACKNOWLEDGEMENT deps.acknowledgedImports: @babel/core -> @babel/preset-typescript matched no finding` and never fails the build, so the list is visibly rotting rather than quietly hiding a new finding. Acknowledgement suppresses reporting, not the runtime behavior it describes: verify that the probe really degrades before adding an entry. It adds no reporting-policy cache input; normal source and packaged workspace-manifest changes still affect content identity. Summaries and unused notices are per closure/platform, so review every selected platform before removing an entry.
