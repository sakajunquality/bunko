...
The current stable release is **0.2.0**, published through GitHub Actions trusted publishing with npm provenance. See [stable publication evidence](validation/v0.2.0.md).

Usage:

```sh
bunx @sakajunquality/bunko@0.2.0 version
bunx @sakajunquality/bunko@latest build .
npm install -g @sakajunquality/bunko@0.2.0
bunko version
```
...
```sh
gh release download v0.2.0 --repo sakajunquality/bunko --dir dist/release
BUNKO_ATTESTATION_SOURCE_DIGEST=e3644475fecfae587db11a8c7dca229dbd6c4ca1 \
  bun scripts/verify-release.ts dist/release v0.2.0
bun scripts/npm-package.ts dist/release dist/npm v0.2.0
