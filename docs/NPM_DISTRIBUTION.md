...
The repository root remains private to prevent publishing the development checkout. `scripts/npm-package.ts` creates a separate, allowlisted package from an existing GitHub release. Its `bunko.js`, licenses, checksums and release provenance are copied without modification. No lifecycle scripts or runtime npm dependencies are included. Bun must already be installed and on PATH; npm does not install it. Supported hosts are Linux and macOS, x64 and arm64. Published 0.1.4 requires Bun >=1.3.11 <1.5; 0.10.0 requires >=1.3.13 <1.5. Version 0.11.0 and subsequent releases require `>=1.3.13 <1.4 || >=1.4.2 <1.5`; packaging older releases preserves their original requirements.

The current stable release is **0.12.5**, published through GitHub Actions trusted publishing with npm provenance. See [stable publication evidence](validation/v0.12.5.md).

Usage:

```sh
bunx @sakajunquality/bunko@0.12.5 version
bunx @sakajunquality/bunko@latest build .
npm install -g @sakajunquality/bunko@0.12.5
bunko version
```
...
```sh
gh release download v0.12.5 --repo sakajunquality/bunko --dir dist/release
BUNKO_ATTESTATION_SOURCE_DIGEST=2ed0b81b03bfcc960631dc83c8fa6c324e68dd9a \
  bun scripts/verify-release.ts dist/release v0.12.5
bun scripts/npm-package.ts dist/release dist/npm v0.12.5
