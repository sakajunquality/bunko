...
The repository root remains private to prevent publishing the development checkout. `scripts/npm-package.ts` creates a separate, allowlisted package from an existing GitHub release. Its `bunko.js`, licenses, checksums and release provenance are copied without modification. No lifecycle scripts or runtime npm dependencies are included. Bun must already be installed and on PATH; npm does not install it. Supported hosts are Linux and macOS, x64 and arm64. Published 0.1.4 requires Bun >=1.3.11 <1.5; 0.8.3 requires >=1.3.13 <1.5.

The current stable release is **0.8.3**, published through GitHub Actions trusted publishing with npm provenance. See [stable publication evidence](validation/v0.8.3.md).

Usage:

```sh
bunx @sakajunquality/bunko@0.8.3 version
bunx @sakajunquality/bunko@latest build .
npm install -g @sakajunquality/bunko@0.8.3
bunko version
```
...
```sh
gh release download v0.8.3 --repo sakajunquality/bunko --dir dist/release
BUNKO_ATTESTATION_SOURCE_DIGEST=ddee9a0d5eb0e2c26ea61ecab32fdd9adbb1f5e8 \
  bun scripts/verify-release.ts dist/release v0.8.3
bun scripts/npm-package.ts dist/release dist/npm v0.8.3
