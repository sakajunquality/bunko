...
6. Run `check-config` and `doctor` with every selected `--asset-context NAME=DIR` binding. Redirect their output to a file so the machine-readable JSON is captured instead of the terminal summary. That JSON can still contain application names, logical paths and, for `doctor`, the selected Bun executable path. Keep it private. Then build with `--push=false --git-metadata=false`, a private output directory, no public registry cache and no automatic artifact upload. Use the build report's entry map to select commands.
