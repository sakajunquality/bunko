...
The static guard rejects global `Bun` API access, `bun` / `bun:*` imports and Bun-only `import.meta` APIs in included source and loaded bundle inputs. Comments, strings, type-only references and lexical local bindings are ignored. `check-config --deep` checks sanitized target entrypoints in bundle mode and permitted target sources in source mode; bundling checks all loaded sources, including dependencies. Unused Bun test/dev files do not block bundle mode. Packaged external JavaScript is checked for both modes, including cache hits and imported dependency layers, with a 64 MiB per-file validation limit. This is conservative: `typeof Bun` detection alone is allowed, but branches actually using Bun APIs are still rejected even when guarded. Dynamic property construction, native behavior and every future API difference cannot be proven statically. Test your final image.
