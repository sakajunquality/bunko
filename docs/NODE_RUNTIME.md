...
`--runtime-kind node` overrides the configured kind. Workspace `bunko.defaults.runtime` is inherited normally. `runtime.node` selects supported major `22` or `24`; otherwise a recognized major in an official Node or distroless base selects that major. Conflicting explicit majors or engine ranges fail before registry access. Without an explicit base, a compatible `engines.node` range selects 24, then 22, with 24 the default when no engine is declared. Custom bases, unversioned tags such as `node:latest`, digest-only Node references and local layouts require an explicit major or supported engine range. Ranges that cannot select a major's `.0.0` baseline need an explicit major and independent base-version verification. This selection does not assert that an arbitrary base satisfies an engine range.
...
The static guard rejects global `Bun` API access, `bun` / `bun:*` imports and Bun-only `import.meta` APIs in included source and loaded bundle inputs. Comments, strings, type-only references and lexical local bindings are ignored. `check-config --deep` checks sanitized target entrypoints in bundle mode and permitted target sources in source mode; bundling checks all loaded sources, including dependencies. Unused Bun test/dev files do not block bundle mode. For both modes, the final app/dependency/asset layers are staged into a contained, temporary filesystem and Bun's Node-target resolver follows the selected entrypoints. Only reachable JavaScript is parsed for API compatibility; unrelated adapters and conditional exports for other runtimes are not rejected merely because they are packaged. This also applies to cache hits and imported dependency layers. Input files remain bounded to 64 MiB; package code and macros are never executed. Simple `typeof Bun` comparisons, conditional expressions, if/else and short-circuit guards are evaluated for Node (including Bun's minified `typeof Bun < "u"` form). Unknown conditions remain conservative. General control-flow reasoning, runtime-selected imports and custom loader behavior are not certified. Dynamic property construction, native behavior and every future API difference cannot be proven statically. Test your final image.
...
Portable-package acceptance on September 17, 2026 used Hono 4.13.8 (`hono/streaming`) and drizzle-orm 0.45.2 together. Both bundle and source images built successfully and their packaged payloads executed with host Node, returning the Hono streamed response and constructing a Drizzle SQL expression. This is package compatibility evidence, not a database-driver or all-adapter certification. Regression fixtures also cover unused Bun adapters, guarded runtime detection, unconditional reachable Bun APIs and cached layers. Shared lexical-scope changes do not expand Bun module-location diagnostics into class heritage; the Node API guard still inspects executable heritage expressions.
