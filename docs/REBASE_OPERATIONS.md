...
With smoke enabled, ordering is Docker preflight, input preparation, local per-platform acceptance, digest publication, attachments, signing, then tag promotion. Preflight and acceptance failures make no registry mutations. Docker image loads default to 300 seconds; use `--smoke-load-timeout` (1..3600 seconds) for larger images or slower daemons. Container commands retain a 60-second deadline. Registry publication after acceptance is not transactional: a later publication/signing failure may leave the digest, and partial tag promotion is possible. Existing immutable tags skipped by the registry remain recorded as skipped; that is not successful movement of a stable tag. No rollback of tags is automatic. Retain the old digest from the report and use an explicitly authorized registry tag operation to roll back. Without smoke, existing rebase behavior is unchanged.
