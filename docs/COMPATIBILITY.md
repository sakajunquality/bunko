...
Unreleased input hardening: npm environment expansion requires a `BUNKO_NPM_*` name or explicit `BUNKO_NPM_CREDENTIAL_ENV` permission, and credentials require a declared host scope. Resolve/apply references and selected workspace roots must stay within `--context`; trusted external inputs require `--allow-external-context`. Project configuration symlinks are rejected before parsing and parser failures omit input contents. Git metadata disables fsmonitor/hooks and system/global configuration; use `--git-metadata=false` when Git inspection is unwanted.
...
See [persisted format compatibility](FORMAT_COMPATIBILITY.md) for ownership capsules, evidence, reports, provenance and cache rollback. In particular, unreleased oversized SBOM evidence v2 requires a reader containing #208; released 0.9.0/0.10.0 cannot rebase that evidence with `--sbom`.
Unreleased image hardening changes: explicit image users must be canonical numeric `uid[:gid]`; replace names such as `nonroot` with `65532:65532`. Named and signed/zero-padded base users now fall back to the nonroot default. Bun bundle images add `--no-install`, so undeclared computed imports no longer download packages at runtime. These changes alter generated image configuration and digests.
