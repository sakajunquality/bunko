...
Development builds after rc.2 require GnuPG's `gpgv` for compile mode as well as runtime injection. Compilation embeds the signature-verified, pinned official Linux Bun runtime through `--compile-executable-path`; it does not delegate runtime downloads to Bun. Supported compile releases are 1.3.11, 1.3.12 and 1.3.13, using x64-baseline or aarch64 glibc assets. Custom or unpinned compiler revisions fail verification. The host Bun executable remains selected by `--bun-path` and its digest remains an input to application caching.
