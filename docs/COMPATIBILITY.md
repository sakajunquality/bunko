...
Bunko v0.12.2 accepts Bun `>=1.3.13 <1.4 || >=1.4.2 <1.5`. The CI matrix pins Bun 1.3.13 and 1.4.2 on Linux and macOS; this is the tested subset, not evidence for every accepted patch. Bun 1.4.0/1.4.1 are rejected before registry access; upgrade to 1.4.2 or pin the immutable v0.10.0 release, which retains `>=1.3.13 <1.5`. Bunko 0.2.0 raises the minimum to 1.3.13. Bunko 0.1.4 remains available for 1.3.11/1.3.12.
...
Bun 1.4 support is available in rc.4 and later; the immutable rc.3 CLI accepts only the previous range. Releases 0.2.0 through 0.10.0 accepted stable host versions `>=1.3.13 <1.5`. Development builds now exclude 1.4.0/1.4.1 as described above. Releases rc.4 through 0.1.4 also accepted Bun 1.3.11/1.3.12. Canaries, prereleases and Bun 1.5 are rejected. Official compile/injection archives are separately pinned for 1.3.13 and 1.4.2, and every archive still requires the embedded trusted GPG signature policy.

Bun 1.4 generates text lockfile version 2. Bunko accepts lock versions 1 and 2 with config version 1, preserving registry-only resolution and integrity requirements. Version 2 was introduced in Bun 1.4.0; select a supported Bun >=1.4.2 for development builds. Builds and `doctor` reject an older compiler before dependency installation or registry access. `check-config` reports the lock version without executing Bun. Existing version 1 locks remain supported and are not automatically rewritten by Bunko. To adopt version 2, regenerate with the selected Bun 1.4 binary in your project and commit the resulting lockfile.
...
Version 0.11.0 cancellation hardening bounds credential helpers and runtime verification even when SIGTERM is ignored. Local archive-loading/apply commands now have a five-minute deadline. Cleanup children have up to five seconds; failed cleanup can still leave uniquely named runtime resources requiring operator cleanup. Packing/decoding pipelines observe invocation cancellation, and tree copies check cancellation between files. This does not promise interrupting a single native filesystem copy mid-file.
Version 0.11.0 input hardening: npm environment expansion requires a `BUNKO_NPM_*` name or explicit `BUNKO_NPM_CREDENTIAL_ENV` permission, and credentials require a declared host scope. Resolve/apply references and selected workspace roots must stay within `--context`; trusted external inputs require `--allow-external-context`. Project configuration symlinks are rejected before parsing and parser failures omit input contents. Git metadata disables fsmonitor/hooks and system/global configuration; use `--git-metadata=false` when Git inspection is unwanted.

Git metadata retains revision/source but omits dirty-state labels and revision-derived tags when configured clean/process filters or indexed submodules would require execution-capable status inspection. A generic incomplete-metadata warning is emitted.
See [persisted format compatibility](FORMAT_COMPATIBILITY.md) for ownership capsules, evidence, reports, provenance and cache rollback. In particular, oversized SBOM evidence v2 requires a reader from 0.11.0 onward; released 0.9.0/0.10.0 cannot rebase that evidence with `--sbom`.
Version 0.11.0 image hardening changes: explicit image users must be canonical numeric `uid[:gid]`; replace names such as `nonroot` with `65532:65532`. Named and signed/zero-padded base users now fall back to the nonroot default. Bun bundle images add `--no-install`, so undeclared computed imports no longer download packages at runtime. These changes alter generated image configuration and digests.

Version 0.12.0 moves to explicit cache packing/policy identities and narrowed workspace input fingerprints, causing a one-time cold cache. Unknown cache layouts/references remain unmanaged and inhibit blob reclamation; legacy/ambiguous locks require manual recovery. See [cache retention](CACHE_RETENTION.md) and [parser maintenance and measurements](PARSER.md) for the retained boundaries and size/scan tradeoff.
