...
`bunko.user` (or `--image-user`) requires canonical numeric `uid[:gid]` values. Explicit `"0:0"` permits root. Without an explicit value, only canonical numeric nonzero base UIDs are inherited; empty, signed/zero-padded IDs and all names (including `nonroot`, which a base can map to UID 0) fall back to `65532:65532`. Numeric users also let Kubernetes evaluate `runAsNonRoot` without resolving names. IDs must be at most 4294967294.
...
`bunko.args` remains application arguments after the entry script. Named entrypoints retain their CMD-based selection while Bun runtime arguments remain in ENTRYPOINT. Bun bundle and source modes append `--no-install` before the entry script. Compile mode embeds a restricted set of execution options through `--compile-exec-argv`; application `args` remain separate. See [compile runtime options](COMPILE.md#runtime-options). Runtime flags do not change bundler resolution decisions made while building bundle-mode code. Arguments are passed as argv entries, never evaluated by a shell. Diagnostics report their count, not their values.
...
Use `bunko check-config . --deep --format text` to validate selected local assets and entrypoints before installing dependencies. It reuses the source walker and configured asset selection rules and reads font contents for local mappings. The deep walk is the build's own source walker over the build's own required inputs, so it inspects exactly the files a build would snapshot: excluded names such as `node_modules` and `.git`, `.bunkoignore` and source-mode `.gitignore` entries and asset exclusions decide what it sees. It reads every file it selects, so an input that cannot be read fails the check as it would fail the copy into a build's snapshot. The default depth does not walk the source tree at all. It creates no staging tree and does not fetch remote assets. Text output names and counts unchecked categories; `valid` covers only the chosen validation depth. URL/hash shape is checked, while remote contents, future generated outputs and runtime compatibility still require a build. See the [cookbook](COOKBOOK.md).
