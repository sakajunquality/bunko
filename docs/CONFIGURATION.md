...
`bunko.args` remains application arguments after the entry script. Named entrypoints retain their CMD-based selection while Bun runtime arguments remain in ENTRYPOINT. Source mode appends `--no-install` before the entry script. Compile mode embeds a restricted set of execution options through `--compile-exec-argv`; application `args` remain separate. See [compile runtime options](COMPILE.md#runtime-options). Runtime flags do not change bundler resolution decisions made while building bundle-mode code. Arguments are passed as argv entries, never evaluated by a shell. Diagnostics report their count, not their values.
