...
A subsequent macOS run still stalled at fifteen seconds before base resolution, so the timeout increase alone was insufficient. The newly frequent executable hashing now uses bounded explicit file reads instead of asynchronous read-stream iteration. Focused build/resolve/cache tests pass locally; CI rechecks this change. This is an observed mitigation, not a proven upstream Bun root-cause diagnosis.
