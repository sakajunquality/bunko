...
The current container recipe packages the published, checksum-verified JavaScript CLI with pinned Bun 1.4.2, GnuPG's `gpgv`, Git and CA certificates. The published 0.9.0 reference is `ghcr.io/sakajunquality/bunko:v0.9.0`, with verified multiarchitecture index `sha256:b29b44b83ecb873b44e672f5c88cb9c1d42c4ef8896bd9121608375473665028`. This recipe supports Bun lockfile v2. Pin the published index digest for reproducible consumption. Container publication and source release publication are separate operations.
...
docker run --rm --read-only --cap-drop=ALL --security-opt=no-new-privileges \
  ...
  ghcr.io/sakajunquality/bunko@sha256:b29b44b83ecb873b44e672f5c88cb9c1d42c4ef8896bd9121608375473665028 \
...
These options are available in v0.9.0. Pin its verified container digest for reproducible use.
...
For the Kubernetes Job example, retain the nonroot/read-only security settings and writable temporary/cache volumes. Use the reviewed v0.9.0 digest shown above. For GKE, configure `serviceAccountName` for workload identity and replace the args with:
