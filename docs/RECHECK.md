...
- SPDX generation stays opt-in, except where the explicit CI policy requires it. Its scope is application/runtime inventory plus explicitly linked base documents, not an OS scanner. Optional `--sbom-evidence` adds bounded lock declarations and observed application states; it does not prove whole-image absence or generate VEX. See [SBOM evidence](SBOM_EVIDENCE.md).
...
1. rc.5 CLI and container publication and independent consumer verification are complete; see [validation evidence](validation/rc5.md). Resume the private GHCR application conformance run after the owner resolves the GitHub billing restriction. npm/bunx distribution and local installation/upgrade/rollback tests are implemented; [rc.5 publication](validation/npm-rc5.md) passed consumer verification. Stable 0.1.0 CLI/container publication, the first npm OIDC publication with verified provenance, and cross-version registry upgrade/rollback are also complete; see [stable evidence](validation/v0.1.0.md). Preserve previous release assets.
...
## Cancellation and memory follow-up

Stream packing/decoding observes invocation cancellation, and source-tree copies check between entries. The work tracked by [#223](https://github.com/sakajunquality/bunko/issues/223) has shipped: build preparation shares authenticated runtime inputs by release/platform/libc asset identity, serializes verification to bound concurrent archive/executable buffers, and stages verified files in invocation-owned scratch space. Targets receive independent metadata for those shared files. A failed target cancels sibling preparation; cleanup waits for active consumers to drain.

Runtime archive/executable verification still uses transient buffers; this is not a fully streaming verifier. Any further streaming optimization must preserve signed-archive and extracted-byte verification, invocation ownership, and cancellation/draining guarantees. Returning a mutable shared-cache path in place of the staged verified file would weaken that contract. Metadata-only remote-cache comparison must not replace blob availability/integrity checks without an explicit trust contract.
