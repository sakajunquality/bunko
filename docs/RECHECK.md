...
Stream packing/decoding observes invocation cancellation, and source-tree copies check between entries. These changes do not yet eliminate runtime archive/executable buffering, implement cross-target download deduplication, or introduce target-local cancellation after a sibling fails. Runtime cache authenticity currently depends on revalidating the signed archive and extracted bytes; returning a mutable shared-cache path without pinning verified bytes would weaken that contract. A follow-up must use invocation-owned verified file handles or immutable staged files, stream those into tar/compile, share by authenticated release/platform/libc identity, and test tampering/eviction while readers are active. Metadata-only remote-cache comparison must not replace blob availability/integrity checks without an explicit trust contract. These remaining optimizations are tracked by #223.
