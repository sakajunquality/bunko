...
- A direct regular executable entrypoint without setuid/setgid bits. Wrapper scripts and symlinked Bun entrypoints require rebuilding.
...
Direct Bun library dependencies must resolve through supported loader search paths, including image-local links. Resolved libraries and the libc loader must have a target-compatible ELF64 shared-object header: little-endian encoding, the selected machine architecture, and supported ELF version/type/OS ABI. Inspection retains at most 64 header bytes per file; text files, truncated headers and foreign-architecture objects fail before publication. A same-named file elsewhere in the image does not satisfy a dependency. The glibc resolver supports RPATH, LD_LIBRARY_PATH, RUNPATH, baseline target-ABI entries in the [GNU loader cache 1.1 format](https://github.com/bminor/glibc/blob/master/sysdeps/generic/dl-cache.h), and standard Debian/Ubuntu or Fedora/RHEL-family default directories. The musl resolver supports LD_LIBRARY_PATH, RUNPATH/RPATH and the [musl architecture-specific path file or default directories](https://git.musl-libc.org/cgit/musl/tree/ldso/dynlink.c). Relative paths, unsupported dynamic tokens, DF_1_NODEFLIB and unsupported cache formats fail closed. ORIGIN expansion is supported in ELF paths and glibc LD_LIBRARY_PATH. Loader configuration files must resolve to captured regular files. These checks establish direct dependency lookup and header compatibility, not complete ELF validity or transitive symbol/ABI compatibility; the explicit contract and application tests remain responsible for those properties.
