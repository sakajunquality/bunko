...
Prepared layouts contain one named multi-platform index (`bunko.local/prepared-base:sha256-…`). Platform aliases are normalized before deduplication, so `linux/arm64,linux/arm64/v8` selects one image. Preparation verifies descriptor digests and sizes for the complete selected graphs; it does not independently decompress every layer to verify its DiffID. Subsequent builds decode the base layers and verify DiffIDs while checking filesystem compatibility. Runtime execution compatibility checks remain separate.
