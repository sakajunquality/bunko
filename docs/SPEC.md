...
The local cache stores CAS blobs and atomic key records under `${XDG_CACHE_HOME:-~/.cache}/bunko/v1`, plus closure plan records under `plans/deps` (see §9) and base inspection records under `base-inspect/<inspection-version>`. Registry caches use `bunko-cache-v1-{deps|assets|app}-<full-key>` tags in the image repository or a specified cache repository. The custom OCI artifact's config includes schema, key, kind, pack format, destination, platform, descriptor, DiffID, inventory, and native metadata.

A base is addressed by an immutable digest, so the filesystem tree the base inspection derives from it is a pure function of that digest and the inspection logic. That tree — every entry's path, type, mode, size and link target, after layer application, whiteouts and implied directories — is stored as one JSON record named by the base digest under a directory named for the inspection version, which is bumped whenever the derivation changes. A record is validated on read against that version, the requested digest and the entry shape inspection could have produced; any mismatch is a miss and the base is inspected again. The record owns no blob and is local only: it is never published to or read from a registry. A hit replaces decoding every base layer, so base layer blobs stay deferred until assembly or publication needs them. `--no-cache` and `--no-local-cache` neither consult nor write a record, and records above the 8 MiB metadata limit are not written. Like a closure plan, a record is a trusted local input: a writer who controls the cache directory can describe a base other than the one the digest names, which is the same capability as supplying that base.
...
Lookup order is local, Registry metadata, then miss; the base inspection is local-only and has no registry step. Local blobs are checked by compressed digest and DiffID. Registry cache bodies are fetched and verified during preparation, with bounded compressed/decompressed sizes. Invalid metadata, unavailable caches, and corrupt bodies cause a diagnostic and miss before publication. Base filesystem validation is separate from application/dependency cache validation and also runs on cache hits.

Layer cache records are saved after independent construction/determinism checks succeed. A base inspection record is not an output record and is exempt: it is written as soon as the inspection that produced it completes, because it restates an immutable base rather than anything this build constructed. Registry cache publication follows successful image publication and applies the same bound across cache artifacts, each of which places its own two blobs serially, so the in-flight requests never multiply. Cache write failure does not undo image success. Application caching, cross-process locks, explicit pruning, and bounded target preparation with `--jobs` are implemented; see the performance contract below.

Determinism verification bypasses persistent layer caches on both runs and compares layers/configs/platform manifests and inventory. It still consults the base inspection cache, because the base is pinned for the whole build and one inspection — cached or fresh — is shared by both iterations, so neither iteration can observe a different base. Bun's download cache may still be reused. Dry-run may use local caches and download packages and may write its report, but performs no Registry writes, export, or loading.
...
A first inspection of a base digest downloads and decodes its layers even when a registry mount or existing blob would have avoided downloading them. It uses the bounded layer decoder and limits inspection to 200,000 tar entries across the selected platform image. Scans are reused for the same pinned manifest across selected contexts and determinism passes, and the resulting metadata map is persisted per base digest as a base inspection record (§6), which is replayed on a later invocation instead of decoding the layers again. Such a record is a layer-cache entry's opposite in one respect: it is written when the inspection completes rather than after construction and determinism checks, because it describes an immutable base rather than an output this build produced.

A replayed inspection therefore performs no decode of its own, and base layer blobs are materialized and verified only where they are actually used. A registry mount or an already-present blob at the destination transfers no bytes and verifies nothing locally; a publication that falls back to uploading, and an OCI layout export, materialize each blob and check its compressed digest and size; a Docker archive export — `--tarball`, and the archive `--local`/`--kind` load through — decodes every layer and verifies its DiffID against the image config. A build that only pushes to a registry that mounts every base layer never reads those bytes at all. Their metadata maps remain in memory until the invocation completes; memory use grows with distinct selected base manifests. Independent application construction reuses the already verified base tree rather than decoding it twice. Use a prepared local base layout to avoid repeated registry reads. The `base-inspect` progress/telemetry phase records this read and decode cost. Historical benchmarks predate this additional validation cost.
