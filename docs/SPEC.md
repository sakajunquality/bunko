...
Closure plan records are published to the same registry cache as `bunko-cache-v1-deps-plan-<full-plan-key>` tags, so the `deps-plan` prefix separates them from the layer tags for retention. A plan artifact carries the same `application/vnd.bunko.cache.v1` artifact type and the OCI empty descriptor as its single layer, because it indexes a layer instead of owning one; its config blob is the validated closure plan record under `application/vnd.bunko.cache.plan.config.v1+json`, and its manifest annotations name the key, the `deps-plan` kind, the plan layout version and the pack format.
...
File hashing is limited to 16 concurrent reads and preserves input order. Closure keys hash every projected file's SHA256, mode, path, links, layout version, toolchain, platform, base digest, workdir, epoch, and pack format; the full lock is excluded, so different sources or patches yielding identical projected bytes reuse one closure layer. Lock validation always runs. A closure plan key, computed before any install from the production dependency inputs (dependency manifest fields, the full lock, patches, noncredential registry settings, install policy, catalogs, reachable workspace source bytes, Bun version/revision, target platform, base digest, libc, destination, epoch, pack format), minus the selected targets' own workspace source bytes, plus the closure inputs (externals, `deps.allowIgnoredScripts`, `deps.undeclaredImports`, target paths, sharedDeps membership, workdir and the closure layout version; `deps.acknowledgedImports` is deliberately absent, because it only filters reporting), indexes the content key a full build produced. When that plan hits and the closure layer it names is still available, the frozen Linux install and the projection are both skipped and the recorded inventory, native metadata, target aliases, package sizes and dependency paths, omitted-addon count, and undeclared/optional-import findings are replayed under the selected policy and the selected acknowledgements; any other outcome projects the closure and records a fresh plan. A selected target is the root of its own closure — the projection starts at the target's declared externals and the target's own files ship in the application layer — so the targets' source bytes are removed from the plan inputs even when other members declare them as dependencies and the production key therefore carries them; every other reachable workspace package, such as a shared library listed in `external`, keeps its bytes in the plan key. A target can still end up inside the projection: a workspace dependency cycle or a self-referencing external reaches it, and under sharedDeps one selected target may simply externalise another with no cycle at all. Whenever the projection contains any selected target, the build records no plan, and a plan whose recorded projection contains any selected target is never reused; those builds always install and project. The plan is an index, never a layer identity: content addressing still decides what a layer is. Bun's extracted download cache remains a trusted build input for closure plans exactly as it is for production dependency keys. Plan lookup checks the managed local cache first and, on a local miss, the ordered registry cache sources (`--cache-from` order, then the cache repository), so a runner whose local cache is empty still reaches the warm path from the repository that already holds the closure layer; a registry plan is written through to the local cache once the layer it names is durable there. A candidate is accepted only once the build can actually use it — its projection contains no selected target and the `deps` record it names resolves, itself possibly from a registry layer — and a source holding an unusable candidate does not end the search, so a later source may still supply a usable plan. Each closure layer key is resolved once per lookup, so several sources naming the same layer cost one lookup and one event. Every record a registry supplies is validated against the expected plan key, destination and platform exactly as a local record is, including the packing rules its replayed aliases must satisfy, and any record that fails is a miss with a reason, never an error. Validation returns a record rebuilt from the fields it checked, so nothing unvalidated is replayed, stored or republished, and a plan is written to the local index only after it passes the same validation a read applies. Registry metadata reads are bounded by the blob idle timeout, so a source that answers and then stalls its body is a miss rather than a hang. Explicit `--cache-from type=local` / `--cache-to type=local` locations carry layer records only. `--no-cache`, `--no-local-cache` and `--verify-deterministic` never consult a plan from any transport: without a managed local cache there is no index for a hit to be written through to, and the flags exist to force a clean local flow. `--no-cache` and `--no-registry-cache` also stop the registry from being consulted at all, and determinism verification and its second iteration always install and project. Production cache hits continue to skip the Linux install.

Publishing a plan follows the layer cache rules exactly: it needs a registry cache write destination, honours `--cache-write=false`, is skipped by dry-run and offline builds, and reports its outcome in `cacheExports` with kind `deps-plan` under the `--cache-export-error` policy, so a failed plan export warns by default and never fails a build. A plan is published only after the `deps` record it names was seen at that destination during the same build, so a destination that rejected the record never gains an index for it; that is ordering rather than a guarantee, because a concurrent prune between the two passes can still strand an index, which readers treat as a miss and reproject.

Every closure plan lookup records a `cache` entry of kind `deps-plan`, with status `local`, `registry`, `miss` or `bypass` and the usual reason. It is a lookup of the closure index rather than of a layer, so consumers that aggregate layer reuse must exclude it; it appears in local-only builds too, and is the one report change a build with no registry cache configured sees. A build that took the plan from a registry logs `Reusing dependency closure (<architecture>) from registry plan`.
