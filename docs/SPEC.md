...
A packaged `.node` file must be a little-endian ELF64 binary for the target architecture; DT_NEEDED is recorded. Prebuilt `.node` files for other platforms that ship in the same package (for example one file per target triple) are omitted from the image and counted in the build log; a package whose `.node` files include none for the target fails. If Bun installs both glibc and musl optional variants, the tree is preserved. Arbitrary base ABI/shared-library checks and native source compilation are not performed. The glibc prebuilt addon in examples/dependencies has run on amd64/arm64.
