...
steps:
  ...
  - uses: sakajunquality/bunko@v0.4.0
    with:
      bun-version: 1.4.2
...
2. `GITHUB_ACTION_REF`, when the ref is version-shaped, such as `v0.4.0`, and `GITHUB_ACTION_REPOSITORY` still names the release repository. Only the spelling of the ref decides this; the runner reports the requested ref without saying whether it is a tag, so a branch named `v9.9.9` selects release v9.9.9 and fails when no such release exists. Refs that are not version-shaped, including `main`, an alias such as `latest` and a commit SHA, and refs reported for a wrapping Action, fall through to the next step.
