...
The migration reduces distribution size and removes the runtime compiler-API dependency, but this scan workload is approximately 1.6 times slower by the median. These numbers are not an end-to-end build speedup claim. Parent links are collected with lexical scopes when needed, instead of eagerly walking every syntax tree twice; child traversal uses Babel's public visitor keys. Keep measuring this tradeoff when updating the parser.
