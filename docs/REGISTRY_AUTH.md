...
Credential selection is validated even in offline builds. Selected credential files are excluded from assets and source fingerprints before network access is disabled; offline mode does not invoke their providers. A refresh lookup never joins an older non-refresh lookup, and late results cannot overwrite refreshed credentials.

Repository-scoped auth keys are not supported by Bunko's host-scoped provider API. Both Docker and Podman configurations reject matching repository-scoped entries rather than broadening their access. In mixed source lists, selected credential helpers remain helper references in the temporary cosign configuration; their returned secrets are not written to that file. Direct metadata transport requires a Bun executable on `PATH`, including when the calling CLI is a compiled executable; the isolated worker starts without proxy or project `.env` settings.
