...
const major = process.env.BUNKO_NODE_MAJOR ?? "24";
if (!["22", "24"].includes(major)) throw new Error("BUNKO_NODE_MAJOR must be 22 or 24");
...
try {
  for (const libc of ["glibc", "musl"]) for (const mode of ["bundle", "source"]) for (const platform of (process.env.BUNKO_SMOKE_PLATFORMS ?? "linux/amd64,linux/arm64").split(",")) {
    ...
    records.push({ major, libc, mode, platform, digest: result.root.digest, actual }); console.error(`Passed ${id}`);
  }
