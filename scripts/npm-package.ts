...
export async function prepareNpmPackage(distribution: string, output: string, version: string) {
  ...
  try {
    ...
    await writeFile(join(output, "package.json"), JSON.stringify({
      ...
      engines: { bun: ">=1.3.11 <1.5" }, os: ["linux", "darwin"], cpu: ["x64", "arm64"],
