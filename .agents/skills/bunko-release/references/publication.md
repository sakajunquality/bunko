...
A failed external command writes private stdout/stderr diagnostics under the helper's temporary directory and reports the path. Inspect these locally to distinguish registry absence, authentication and network errors. Do not paste raw logs into reports or PRs: they may contain signed URLs or credentials. Reports deliberately retain only accepted identities and outcomes.

The npm helper stages verified bytes before running bunx with `--no-install`. Run the checklist’s direct fresh-cache bunx checks separately; the helper does not replace them. Failure logs include both output streams and timeout diagnostics and must remain private. The container helper assumes a locally accessible Docker endpoint; named contexts requiring stored TLS credentials need separate anonymous-consumer setup.
