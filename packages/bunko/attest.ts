...
export function spdx(name: string, image: PlatformResult, timestamp: number, runtime?: { version: string; revision: string; embedded: boolean }, evidence?: BuildEvidence) {
  if (image.nodeRuntime) runtime = undefined;
  ...
  const document = { spdxVersion: "SPDX-2.3", dataLicense: "CC0-1.0", SPDXID: "SPDXRef-DOCUMENT", name: `${name}-${image.platform.architecture}`,
    ...
    packages: [root, ...packages, ...(image.nodeRuntime ? [{ SPDXID: "SPDXRef-Node-Runtime", name: "node", versionInfo: image.nodeRuntime.version, downloadLocation: "NOASSERTION", filesAnalyzed: false, licenseConcluded: "NOASSERTION", licenseDeclared: "NOASSERTION", copyrightText: "NOASSERTION", comment: "Declared Node major for the base runtime; version and execution are not independently verified", externalRefs: [{ referenceCategory: "PACKAGE-MANAGER", referenceType: "purl", referenceLocator: `pkg:generic/node@${image.nodeRuntime.version}` }] }] : []), ...(runtime ? [{ SPDXID: "SPDXRef-Bun-Runtime", name: "bun", versionInfo: runtime.version, downloadLocation: release?.url ?? "NOASSERTION", ...(release ? { checksums: [{ algorithm: "SHA256", checksumValue: release.archiveDigest.slice(7) }] } : {}), filesAnalyzed: false, licenseConcluded: "NOASSERTION", licenseDeclared: "NOASSERTION", copyrightText: "NOASSERTION", comment: release ? `${image.compileRuntime ? "Embedded" : "Injected"} signed release; policy ${release.policy}; signer ${release.signer}; release revision ${release.releaseRevision}; runtime execution not verified` : `${runtime.embedded ? "Embedded" : "Expected base"} Bun runtime revision ${runtime.revision}; custom base runtime identity is not independently verified`, externalRefs: [{ referenceCategory: "PACKAGE-MANAGER", referenceType: "purl", referenceLocator: `pkg:generic/bun@${runtime.version}` }] }] : [])], relationships: [
      ...
      ...(image.nodeRuntime ? [{ spdxElementId: root.SPDXID, relationshipType: "DEPENDS_ON", relatedSpdxElement: "SPDXRef-Node-Runtime" }] : []),
