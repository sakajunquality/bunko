import { runAbortScope, throwIfCancelled } from "../runtime/invocation.ts";
/** Limit open file streams while preserving input order and draining on failure. */
export async function mapJobs<T, R>(values: readonly T[], jobs: number, map: (value: T) => Promise<R>, options: { cancelOnFailure?: boolean } = {}): Promise<R[]> {
  if (!Number.isSafeInteger(jobs) || jobs < 1 || jobs > 32) throw new Error("jobs must be an integer from 1 to 32");
  const run = async (abort: (reason: unknown) => void) => {
  ...
  await Promise.all(Array.from({ length: Math.min(jobs, values.length) }, async () => {
    while (!failed && next < values.length) {
      ...
      catch (error) { if (!failed) { failed = true; failure = error; abort(error); } }
    }
  }));
  if (failed) throw failure;
  return results;
  };
  return options.cancelOnFailure ? runAbortScope(run) : run(() => {});
}
