...
export async function dependencyClosure(root: string, prefix: string, platform: Platform, projects: Project[]) {
  ...
  const entries: TarEntry[] = [], inventory: InventoryEntry[] = [], native: NativeBinary[] = [], undeclared: UndeclaredImport[] = [], optionalUndeclared: UndeclaredImport[] = [];
  ...
  return { entries, inventory, native, undeclared, optionalUndeclared, omitted: ledger.finish(), aliases: new Map([...roots].map(([path, edges]) => [path, aliases(edges, `${prefix}/node_modules`)])) };
}
