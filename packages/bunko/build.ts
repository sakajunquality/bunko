    ...
    async function runBuild(iteration: number): Promise<PlatformResult[]> {
      ...
      for (const [index, platform] of project.platforms.entries()) {
        ...
        const image = await assembleImage(store, base, layers, {
          ...
          platform, epoch: timestamp, entrypoint: project.mode === "compile" ? [`${project.workdir}/${application.entry}`] : project.entrypoints ? [project.bunPath, ...project.runtimeArgs, ...(project.runtimeKind !== "node" ? ["--no-install"] : [])] : [project.bunPath, ...project.runtimeArgs, ...(project.runtimeKind !== "node" ? ["--no-install"] : []), `${project.workdir}/${application.entry}`],
