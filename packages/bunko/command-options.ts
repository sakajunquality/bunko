...
export function validateCommandOptions(command: string, names: string[]): void {
  const build = "runtime-arg registry-mirror define otel asset-context base-sbom deps-verify-key supply-chain-policy deps-map registry-config publish-concurrency progress image-label image-annotation image-user image-refs repo bare tag tag-conflict push oci-layout tarball local kind kind-cluster base base-layout platform bun-path cache cache-dir cache-repo cache-from cache-to cache-write cache-export-error local-cache registry-cache app-cache install-cache asset-cache runtime-libc runtime-inject runtime-cache insecure-registry dry-run reproducible verify-deterministic git-metadata index jobs mode sbom provenance sign-key cosign-path report target deps-strategy shared-deps module-locations";
