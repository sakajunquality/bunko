...
import { Publisher, PublicationError, type Publication } from "./publish.ts";
...
export async function publishArtifacts(publisher: Publisher, store: BlobStore, artifacts: Artifact[], record: (publication: Publication, elapsedMs: number) => void = () => {}): Promise<void> {
  for (const item of artifacts) {
    // An attachment costs more than its own blobs and manifest: referrers probing, bounded
    // verification retries and a fallback index write all belong to the reported elapsed time,
    // including when one of them fails.
    const started = performance.now();
    let publication: Publication | undefined;
    try {
      publication = await publisher.publish(store, item.manifest, [], new Map(item.blobs.map((d) => [d.digest, "attestation"])));
      ...
      if (publication) record(publication, Math.round(performance.now() - started));
    }
  }
}
