...
import { resvgFonts, canvasFont } from "./rendering.mjs";
const recipe = process.env.FONT_DISCOVERY ?? "fontconfig";
...
const fallbackEmoji = canvas("😀🎉", canvasFont);
...
const system = render(resvgFonts(recipe));
...
console.log(JSON.stringify({ status: "passed", recipe, arch: process.arch, canvasCJK: digest(automatic), canvasColorEmoji: digest(automaticEmoji), resvgCJK: digest(system), canvasSystemDiscovery: true, explicitRegistrationMatches: true }));
