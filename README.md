...
Build OCI images from Bun projects without a Dockerfile or Docker daemon. Inspired by Go's [ko](https://ko.build/). Bunko also supports building with Bun and [running on Node](docs/NODE_RUNTIME.md); Bun remains the default runtime.

**v0.12.5** supports standalone apps and Bun workspaces, Bun 1.4, bundle/source/compile modes with glibc or Alpine/musl runtimes and module-location diagnostics, optional signed Bun runtime injection, npm dependencies, explicit runtime externals, Registry publication, dependency and asset caching, multiple platforms, Docker/kind loading, and YAML/JSON resolution. Docker-compatible credentials remain the default for GHCR, Google Artifact Registry, Docker Hub and ECR. GitHub, Google, Podman and AWS credential sources are opt-in. Native private ECR authentication was verified with GitHub OIDC. Node runtime images, keyless signing and SBOM build evidence are also available with explicit configuration. See the [Registry matrix](docs/REGISTRIES.md) for the distinction between implemented authentication and verified service interoperability.

Start with the [task-oriented cookbook](docs/COOKBOOK.md). See the [feature guide](docs/FEATURES.md), [0.12.5 release notes](docs/RELEASE_NOTES.md), and [comparison with ko and BuildKit](docs/COMPARISON.md). Review the documented compatibility and trust boundaries before adopting it.

## Install a release

Download `bunko.js`, `SHA256SUMS`, `LICENSE`, `THIRD_PARTY_NOTICES.md`, and `PROVENANCE.jsonl` from the [v0.12.5 release](https://github.com/sakajunquality/bunko/releases/tag/v0.12.5) into the same directory. Install Bun 1.4.2 (or another supported version), then verify the files before running the CLI:
...
[Completed-image acceptance](examples/image-acceptance/README.md) demonstrates container-structure-test and Compose probes against working images and deliberate silent/runtime failures. [Cross-device cache acceptance](docs/CROSS_DEVICE_VALIDATION.md) requires separate work/cache mounts and checks cold builds, warm reuse, and final asset contents and modes through the bundled CLI.
