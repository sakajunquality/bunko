...
Bunko builds trusted source and dependency inputs on the host. Its child-process environment and path checks are not an operating-system sandbox. Use isolated runners for untrusted projects. Registry caches and their writers must be trusted; digest checks alone do not prove the claimed build inputs. Metadata is self-reported unless separately verified against a trusted producer policy.
