...
describe("image composition", () => {
  ...
  test("replaces an inherited root user with nonroot, inherits other users, and honours explicit settings", () => {
    for (const User of ["+0", "-0", "+0:+0", "toor", "nonroot", "0", "0:0", "00", "000:001", "00:00", "root", "root:root", "root:0", "0:root", "0:1000", ""]) expect(imageConfig({ ...base, config: { User } }, [], options).config?.User).toBe("65532:65532");
    expect(imageConfig({ ...base, config: {} }, [], options).config?.User).toBe("65532:65532");
    for (const User of ["1000", "1000:0", "1000:1000", "65532:65532"]) expect(imageConfig({ ...base, config: { User } }, [], options).config?.User).toBe(User);
    expect(imageConfig({ ...base, config: { User: "0" } }, [], { ...options, user: "0:0" }).config?.User).toBe("0:0");
    expect(() => imageConfig(base, [], { ...options, user: "root" })).toThrow("numeric");
    expect(imageConfig(base, [], { ...options, user: "2000:2000" }).config?.User).toBe("2000:2000");
    expect(() => imageConfig(base, [], { ...options, user: "00:00" })).toThrow("numeric");
