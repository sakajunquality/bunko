...
test("stable Bun 1.3 and 1.4 declarations retain explicit range boundaries", () => {
  for (const version of ["1.3.13", "1.3.14", "1.4.2", "1.4.3"]) {
  ...
  for (const version of ["1.3.10", "1.3.11", "1.3.12", "1.4.0", "1.4.1", "1.5.0", "2.0.0", "1.4.0-canary", "1.4.02", "^1.4.0"]) {
...
test("retired toolchains fail before registry access", async () => {
  ...
  let requests = 0;
  for (const version of ["1.3.11", "1.3.12", "1.4.0", "1.4.1"]) {
    const executable = join(root, `bun-${version}`);
    await writeFile(executable, `#!/bin/sh\nprintf '${version}+012345678\\n'\n`); await chmod(executable, 0o755);
    await expect(selectToolchain(executable)).rejects.toThrow("Bun >=1.3.13 <1.4 || >=1.4.2 <1.5");
    await expect(build({ path: fixture.source, bunPath: executable, push: false, output: join(root, "out"), registry: { fetcher: async () => { requests++; throw new Error("No network expected"); } } })).rejects.toThrow("Bun >=1.3.13 <1.4 || >=1.4.2 <1.5");
...
test("toolchain mismatch messages name the selected binary, both versions and the declaration source", async () => {
  ...
  const pinned = toolchainRequirements([{ packageManager: "bun@1.4.2" }]);
  expect(pinned).toEqual({ version: "1.4.2", versionSource: "package.json#packageManager", revision: undefined, ranges: [], rangeSources: [] });
  expect(() => assertToolchain(pinned, selected)).toThrow("Selected Bun 1.3.13 (/opt/bun/bin/bun) does not match the declared version 1.4.2 (package.json#packageManager). Install Bun 1.4.2 and select it with --bun-path, or change the declaration.");
  expect(() => assertToolchain(toolchainRequirements([], { version: "1.4.2" }), selected)).toThrow("declared version 1.4.2 (bunko.toolchain.version)");
  ...
  const ranged = toolchainRequirements([{ engines: { bun: ">=1.3.13" } }, { engines: { bun: ">=1.4.2" } }], undefined, ["package.json", "services/api/package.json"]);
  expect(ranged.ranges).toEqual([">=1.3.13", ">=1.4.2"]); expect(ranged.rangeSources).toEqual(["package.json#engines.bun", "services/api/package.json#engines.bun"]);
  expect(() => assertToolchain(ranged, selected)).toThrow("Selected Bun 1.3.13 (/opt/bun/bin/bun) does not satisfy engines.bun >=1.4.2 (services/api/package.json#engines.bun). Install a Bun version in that range and select it with --bun-path, or change the declaration.");
  expect(() => toolchainRequirements([{ packageManager: "bun@1.3.13" }, { packageManager: "bun@1.4.2" }], { version: "1.4.2" }, ["package.json", "services/api/package.json"])).toThrow("Conflicting Bun toolchain version declarations: 1.4.2 (bunko.toolchain.version), 1.3.13 (package.json#packageManager), 1.4.2 (services/api/package.json#packageManager)");
  ...
  member.packageManager = "bun@1.4.2"; member.engines = { bun: ">=1.3.13 <1.5" }; await writeFile(memberPath, JSON.stringify(member));
