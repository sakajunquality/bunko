...
test("release assets carry matching versions, checksums, and parser licenses", async () => {
  ...
  expect(bundle).toContain("Copyright Eemeli Aro"); expect(bundle).toContain("Copyright (C) 2012-2014 by various contributors");
  ...
  const parserLicense = (await readFile("node_modules/@babel/parser/LICENSE", "utf8")).trim();
  expect(notices).toContain(parserLicense); expect(bundle).toContain(parserLicense);
  expect(notices).toContain(`## @babel/parser ${metadata.devDependencies["@babel/parser"]}`);
